"""
Script de Produção com Atualização em Tempo Real - QuantumTrader ML
Sistema com callbacks otimizados para dados em tempo real
"""
...
# Configurar logging
log_file = f'logs/production/realtime_{datetime.now().strftime("%Y%m%d_%H%M%S")}.log'
...
class RealtimeTradingSystem:
    ...
    def initialize(self):
        ...
        try:
            ...
            result = self.dll.DLLInitializeLogin(
                ...
                self.callback_refs['price'],  # Callback de preço
                self.callback_refs['trade'],  # Callback de trades
                self.callback_refs['book'],   # Callback de book completo
                None,
    ...
    def _create_callbacks(self):
        """Cria callbacks completos para dados em tempo real"""
        ...
        # Callback de preço em tempo real
        ...
        def priceCallback(assetId, date, price, volume, qtd, side, hora):
            try:
                if price > 0 and price < 10000:
                    ...
                    if self.price_updates_count % 10 == 0:
                        self.logger.debug(f"[PRICE] {price:.2f} vol={volume} qtd={qtd}")
        ...
        def tradeCallback(assetId, date, price, volume, qtd, side, hora):
            try:
                if price > 0 and price < 10000:
                    ...
                    # Log trades grandes
                    if volume > 50:
                        self.logger.debug(f"[TRADE] Big: {price:.2f} vol={volume} side={trade_info['side']}")
        ...
        def bookCallback(assetId, lado, buyArray, sellArray):
            try:
                ...
                if self.bid_price > 0 and self.ask_price > 0:
                    ...
                    if spread < 0.5:  # Spread muito apertado
                        self.logger.debug(f"[BOOK] Tight spread: {spread:.2f}")
    ...
    def subscribe_ticker(self, ticker="WDOU25"):
        """Subscreve ticker com todas as flags necessárias"""
        try:
            ...
            # Ativar callbacks de tempo real
            try:
                # Tentar ativar book em tempo real
                self.dll.SubscribeMarketBook(c_wchar_p(ticker))
                self.logger.info("[OK] Book em tempo real ativado")
            except:
                self.logger.warning("Não foi possível ativar book em tempo real")
                
            # Solicitar histórico inicial
            try:
                self.dll.GetHistory(
                    c_wchar_p(ticker),
                    c_int(0),  # Periodicidade (0 = tick)
                    c_int(1),  # Qtd barras
                    c_int(0),  # Ajuste
                    c_wchar_p("")  # Data inicial
                )
                self.logger.info("[OK] Histórico solicitado")
            except:
                pass
    ...
    def _calculate_features(self):
        ...
        try:
            ...
            features['price_current'] = self.current_price  # Usar preço em tempo real
            ...
            # Momentum
            if len(closes) >= 10:
                features['momentum_10'] = (self.current_price / closes[-10]) - 1
            else:
    ...
    def run_strategy(self):
        ...
        self.logger.info("[STRATEGY] Iniciando estratégia ML com dados em tempo real")
        ...
        while self.is_running:
            try:
                ...
                time.sleep(0.1)  # Loop mais rápido para dados em tempo real
...
def main():
    ...
    print("\n" + "="*60)
    print("QUANTUM TRADER ML - PRODUÇÃO TEMPO REAL")
    print("="*60)
    ...
    try:
        ...
        if system:
            ...
            logger.info(f"Atualizações de preço: {system.stats['price_updates']}")
            logger.info("="*60)
