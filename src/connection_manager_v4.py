        ...
        try:
            ...
            if init_result == NResult.NL_OK:
                ...
                if self._wait_for_connections(timeout=30):
                    self.connected = True
                    self.logger.info("✅ Conexão v4.0.0.30 estabelecida com sucesso!")
                    return True
                else:
        ...
        def history_callback(asset_id, date, trade_number, price, vol, qtd,
            ...
            try:
                ...
                if self._historical_data_count == 0:
                    ...
                    self.logger.info(f"📊 {self._historical_data_count} dados recebidos...")
        ...
        try:
            ...
            if hasattr(self.dll, 'SetOrderCallback'):
                ...
                if result == NResult.NL_OK:
                    self.logger.info("✅ SetOrderCallback configurado com sucesso")
                else:
                    self.logger.warning(f"⚠️ SetOrderCallback retornou: {result}")
            
            if hasattr(self.dll, 'SetOrderHistoryCallback'):
                result = self.dll.SetOrderHistoryCallback(order_history_callback_v2)
                if result == NResult.NL_OK:
                    self.logger.info("✅ SetOrderHistoryCallback configurado com sucesso")
                else:
                    self.logger.warning(f"⚠️ SetOrderHistoryCallback retornou: {result}")
            ...
            if hasattr(self.dll, 'SetTradeCallbackV2'):
                ...
                if result == NResult.NL_OK:
                    self.logger.info("✅ SetTradeCallbackV2 configurado")
        ...
        try:
            ...
            if final_count > 0:
                ...
                if self._is_historical_data_complete():
                    ...
                    self.logger.error("❌ Timeout e dados históricos incompletos")
                    return False
            else:
    ...
    def _is_historical_data_complete(self) -> bool:
        ...
        try:
            ...
            # Modo normal (produção)
                ...
                self.logger.info(f"📊 Último dado histórico: {self._last_historical_timestamp.strftime('%H:%M')} | "
                ...
                if time_diff <= 10:
                    ...
                    self.logger.info(f"⏳ Dados ainda defasados em {time_diff:.1f} minutos")
                    return False
