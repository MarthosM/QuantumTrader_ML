...
class MLFeatures:
    ...
    def _calculate_advanced_volatility_features(self, candles: pd.DataFrame, features: pd.DataFrame):
        ...
        try:
            ...
            # Estimador eficiente que usa OHLC - Fórmula padrão corrigida
            try:
                # Calcular logaritmos necessários (fórmula padrão Garman-Klass)
                ln_ho = np.log(candles['high'] / candles['open'])
                ln_lo = np.log(candles['low'] / candles['open'])  
                ln_co = np.log(candles['close'] / candles['open'])
                
                # Fórmula padrão Garman-Klass
                gk_estimator = ln_ho * (ln_ho - ln_co) + ln_lo * (ln_lo - ln_co)
                
                # Períodos múltiplos para Garman-Klass
                for period in [10, 20]:
                    gk_rolled = gk_estimator.rolling(period).mean()
                    features[f'gk_vol_{period}'] = np.sqrt(gk_rolled) * np.sqrt(252)  # Anualizada
                
                self.logger.debug("Garman-Klass volatility calculada com sucesso")
                
            except Exception as e:
                self.logger.error(f"Erro no cálculo Garman-Klass: {e}")
                # Em caso de erro, criar features com zero para não quebrar o sistema
                for period in [10, 20]:
                    features[f'gk_vol_{period}'] = pd.Series(0.0, index=candles.index)
