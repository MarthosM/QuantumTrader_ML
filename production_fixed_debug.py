"""
Sistema de Produção com Debug Detalhado - QuantumTrader ML
"""
...
logger = logging.getLogger('ProductionDebug')
...
class ProductionDebugSystem:
    ...
    def _create_all_callbacks(self):
        ...
        def tinyBookCallBack(assetId, price, qtd, side):
            ...
            if price > 0 and price < 10000:
                ...
                # Log a cada mudança de preço ou 1000 callbacks
                if self.callbacks['tiny_book'] % 1000 == 0 or abs(price - old_price) > 0.5:
    ...
    def _calculate_features(self):
        ...
        self.logger.debug(f"[FEATURES] Iniciando cálculo. Candles: {len(self.candles)}, Price: {self.current_price}")
        
        if len(self.candles) < 20:
            self.logger.debug(f"[FEATURES] Poucos candles: {len(self.candles)} < 20")
            return None
        ...
        try:
            ...
            self.logger.error(f"[FEATURES] Erro ao calcular: {e}")
            import traceback
            traceback.print_exc()
            return None
    ...
    def _make_prediction(self):
        ...
        self.logger.debug("[PREDICTION] Iniciando predição")
        
        if not self.models:
            self.logger.debug("[PREDICTION] Sem modelos carregados")
            return None
            
        try:
            features = self._calculate_features()
            if not features:
                self.logger.debug("[PREDICTION] Features não calculadas")
                return None
            ...
            for model_name, model in self.models.items():
                try:
                    ...
                    if not feature_list:
                        self.logger.debug(f"[PREDICTION] {model_name} sem lista de features")
                        continue
                    ...
                    self.logger.debug(f"[PREDICTION] {model_name}: pred={pred:.3f}, conf={conf:.3f}")
                    
                except Exception as e:
                    self.logger.debug(f"[PREDICTION] Erro em {model_name}: {e}")
                    
            if not predictions:
                self.logger.debug("[PREDICTION] Nenhuma predição gerada")
                return None
            ...
            self.logger.debug(f"[PREDICTION] Final: {result}")
            return result
            
        except Exception as e:
            self.logger.error(f"[PREDICTION] Erro fatal: {e}")
            import traceback
            traceback.print_exc()
            return None
    ...
    def run_strategy(self):
        ...
        self.logger.info("[STRATEGY] Iniciando estratégia ML com DEBUG")
        ...
        # Timeouts reduzidos
        prediction_interval = 10  # Reduzido de 30 para 10
        status_interval = 60
        ...
        while self.is_running:
            try:
                ...
                if (current_time - self.stats['last_debug']) > 5:
                    self.logger.debug(f"[LOOP] Price: {self.current_price}, Candles: {len(self.candles)}, "
                                    f"LastUpdate: {current_time - self.last_price_update:.1f}s ago")
                    self.stats['last_debug'] = current_time
                ...
                # Predição a cada 10 segundos (reduzido)
                if (current_time - last_prediction_time) > prediction_interval and len(self.candles) >= 20:
                    self.logger.debug("[STRATEGY] Tentando fazer predição...")
                    ...
                    if prediction:
                        ...
                        self.logger.debug("[STRATEGY] Predição retornou None")
                ...
                self.logger.error(f"[STRATEGY] Erro no loop: {e}")
                import traceback
                traceback.print_exc()
                time.sleep(5)
    ...
    def _log_status(self):
        ...
        if self.stats['trades'] > 0:
            ...
            self.logger.info(f"Trades: {self.stats['trades']} | Win Rate: {win_rate:.1f}% | P&L: R$ {self.stats['total_pnl']:.2f}")
        else:
            self.logger.info(f"Trades: 0 | Win Rate: 0.0% | P&L: R$ 0.00")
...
def main():
    ...
    print("\n" + "="*60)
    print("QUANTUM TRADER ML - PRODUÇÃO DEBUG")
    print("="*60)
    ...
    try:
        ...
        print("\n" + "="*60)
        print(f"SISTEMA OPERACIONAL (DEBUG)")
