...
Com progresso detalhado usando tqdm
"""
...
class CSV5MCompleteTrainer:
    """Treina modelos completos com 5M registros e progresso detalhado"""
    ...
    def create_advanced_features(self, df: pd.DataFrame) -> pd.DataFrame:
        """Cria features avançadas com progresso"""
        ...
        feature_tasks = [
            ...
            ("Agent Behavior", self._create_agent_features),
            ("Temporal", self._create_temporal_features),
            ("Padrões Avançados", self._create_pattern_features)
        ]
    ...
    def _create_price_features(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        # Retornos múltiplos
        with tqdm([1, 2, 5, 10, 20, 30, 50, 100, 200], desc="  Retornos") as pbar:
            for period in pbar:
                features[f'returns_{period}'] = features['price'].pct_change(period)
        
        # Volatilidade
        with tqdm([10, 20, 30, 50, 100, 200], desc="  Volatilidade") as pbar:
            for window in pbar:
        ...
        features['momentum_50_200'] = features['price'].rolling(50).mean() / features['price'].rolling(200).mean() - 1
    ...
    def _create_volume_features(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        # Volume profiles
        with tqdm([20, 50, 100, 200, 500], desc="  Volume MA") as pbar:
            for window in pbar:
        ...
        # VWAP múltiplo
        with tqdm([100, 500, 1000], desc="  VWAP") as pbar:
            for window in pbar:
    ...
    def _create_flow_features(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        # Flow imbalance
        with tqdm([20, 50, 100, 200, 500], desc="  Flow imbalance") as pbar:
            for window in pbar:
        ...
        features['buyer_streak'] = (features['is_buyer_aggressor'].groupby(
            (features['is_buyer_aggressor'] != features['is_buyer_aggressor'].shift()).cumsum()
        ).cumcount() + 1) * features['is_buyer_aggressor']
        
        features['seller_streak'] = (features['is_seller_aggressor'].groupby(
            (features['is_seller_aggressor'] != features['is_seller_aggressor'].shift()).cumsum()
        ).cumcount() + 1) * features['is_seller_aggressor']
        
        return features
    
    def _create_agent_features(self, df: pd.DataFrame) -> pd.DataFrame:
        """Features de comportamento de agentes"""
        features = pd.DataFrame(index=df.index)
        ...
        # Agent concentration (HHI) - simplificado
        print("    Calculando concentração...")
        
        # Usar apenas algumas janelas para velocidade
        for window in tqdm([100, 500], desc="  HHI"):
            # Amostragem a cada N pontos
            sample_rate = max(1, window // 50)
            sample_indices = range(window, len(df), sample_rate)
            ...
            for i in sample_indices:
                ...
                buyer_hhi.append((buyer_counts / len(window_buyers)).pow(2).sum())
                seller_hhi.append((seller_counts / len(window_sellers)).pow(2).sum())
            ...
            features[f'buyer_hhi_{window}'] = buyer_series.reindex(range(len(df))).interpolate(method='linear').fillna(0)
            features[f'seller_hhi_{window}'] = seller_series.reindex(range(len(df))).interpolate(method='linear').fillna(0)
    ...
    def _create_pattern_features(self, df: pd.DataFrame) -> pd.DataFrame:
        """Features de padrões avançados"""
        features = pd.DataFrame(index=df.index)
        
        # Large trades
        with tqdm([75, 90, 95, 99], desc="  Large trades") as pbar:
        ...
        features['price_efficiency'] = features.get('returns_1', 0).rolling(100).std() / features.get('returns_10', 1).rolling(100).std()
        
        # Trade size clustering
        features['avg_trade_size'] = df['<qty>'].rolling(100).mean()
        features['trade_size_std'] = df['<qty>'].rolling(100).std()
        features['trade_size_cv'] = features['trade_size_std'] / features['avg_trade_size'].clip(lower=1)
    ...
    def train_ensemble_models(self, features: pd.DataFrame, targets: dict):
        ...
        weights = {'LightGBM': 0.35, 'XGBoost': 0.35, 'Random Forest': 0.2, 'Gradient Boosting': 0.1}
        
        for model_name, pred in predictions.items():
            ensemble_pred += weights.get(model_name, 0.25) * pred
    ...
    def _train_random_forest(self, X_train, y_train, X_test, y_test):
        ...
        for i in tqdm(range(n_estimators), desc="  Árvores RF"):
            tree = clone(model)
            tree.fit(X_train, y_train)
            estimators.append(tree.estimators_[0])
    ...
    def save_results(self, feature_importance: pd.DataFrame):
        ...
        output_dir = Path('models/csv_5m_complete')
...
def main():
    ...
    print("PIPELINE COMPLETO - 5 MILHÕES DE REGISTROS")
    print("Com progresso detalhado e múltiplos modelos\n")
    
    csv_path = r"C:\Users\marth\Downloads\WDO_FUT\WDOFUT_BMF_T.csv"
    
    # Criar trainer
    trainer = CSV5MCompleteTrainer(csv_path)
